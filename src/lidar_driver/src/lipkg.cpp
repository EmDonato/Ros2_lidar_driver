...
bool LiPkg::AnalysisOne(uint8_t byte)
{
  ...
  return false;  // Returning here is required when no complete packet is available.
}
...
void LiPkg::ToLaserscan(std::vector<PointData> src)
{
  ...
  output.scan_time = dt / 1000.0;
  output.time_increment = beam_size > 0 ? output.scan_time / beam_size : 0.0;
